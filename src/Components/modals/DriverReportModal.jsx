...
const INFOS = {
  ...
  shipping_companies: { name: "لیست شرکتهای حمل کار شده", icon: "truck-fast" },
...
const DriverReportModal = ({ open, onClose, data }) => {
  ...
  const {
    ...
    ["driver-report", data?.account_id],
    () =>
      axiosApi({ url: `driver-report/${data?.account_id}` }).then(
        (res) => res.data.Data
      ),
    {
      staleTime: 24 * 60 * 60 * 1000,
      enabled: open && !!data?.account_id,
    }
  );
      ...
      {isLoading || isFetching ? (
        ...
        <Box>
          <Typography mt={2} variant="h5" fontWeight={700}>
            گزارش راننده - {data?.person?.first_name ?? ""}{" "}
